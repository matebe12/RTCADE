...
- 이 프로젝트는 수익 창출을 목적으로 하지 않으며, WebRTC 넷플레이, 상태 동기화, iframe 기반 EmulatorJS 연동을 보여주기 위한 포트폴리오용 기술 데모입니다.
- 깃 저장소에는 상용 게임 ROM, BIOS 파일을 직접 커밋하지 않는 전제입니다.
- 별도의 다운로드 UI는 제공하지 않지만, 운영자가 `server/roms/`에 파일을 두면 서버가 해당 파일을 클라이언트에 서빙해 실행할 수 있습니다.
- 공개 배포 또는 데모 운영 시에는 직접 배포 권한이 있는 홈브루, 퍼블릭 도메인, 또는 명시적으로 허가받은 콘텐츠만 사용해야 합니다.
- 이 저장소를 사용하는 운영자는 자신이 업로드하거나 호스팅하는 ROM/BIOS 및 관련 자산의 저작권, 라이선스, 배포 책임을 직접 부담합니다.
...
이 섹션은 로컬 테스트 또는 포트폴리오 데모 환경에서 ROM/BIOS를 연결하는 방법입니다. 프로젝트 의도는 수익형 게임 서비스 운영이 아니라 기술 시연이므로, 데모에는 기본적으로 권리 확인이 끝난 샘플만 사용하는 구성을 전제로 합니다.

- 깃 저장소 자체에는 ROM/BIOS를 직접 커밋하지 않는 전제입니다.
- 이 폴더에 넣은 파일은 서버에서 `/roms/*` 경로로 직접 서빙됩니다.
- 공개 데모에는 홈브루, 퍼블릭 도메인, 또는 명시적으로 허가된 콘텐츠만 사용하세요.
- 개인 테스트나 비공개 운영이라도 업로드·호스팅한 파일의 권리 책임은 운영자에게 있습니다.
...
예전 배포에서 남은 미종료 게임 세션을 한 번 정리해야 하면 [docs/cleanup-stale-game-sessions.sql](docs/cleanup-stale-game-sessions.sql)을 먼저 조회용으로 실행한 뒤, 결과를 확인하고 같은 파일의 UPDATE 구문을 실행하면 됩니다.
